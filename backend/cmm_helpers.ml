...
let rec unbox_vec128 dbg ~aligned =
  let memory_chunk =
    if aligned then Onetwentyeight_aligned else Onetwentyeight_unaligned
  in
  map_tail ~kind:Any (function
    | Cop (Calloc _, [Cconst_natint (hdr, _); c], _)
    ...
    | Cconst_symbol (s, _dbg) as cmm -> (
      ...
      | _ -> Cop (Cload (memory_chunk, Immutable), [cmm], dbg))
    | Cregion e as cmm -> (
      (* It is valid to push unboxing inside a Cregion except when the extra
         unboxing logic pushes a tail call out of tail position *)
      match
        map_tail ~kind:Any
          (function
            | Cop (Capply (_, Rc_close_at_apply), _, _) -> raise Exit
            | Ctail e -> Ctail (unbox_vec128 dbg e ~aligned)
            | e -> unbox_vec128 dbg e ~aligned)
          e
      with
      | e -> Cregion e
      | exception Exit -> Cop (Cload (memory_chunk, Immutable), [cmm], dbg))
    | Ctail e -> Ctail (unbox_vec128 dbg e ~aligned)
    | cmm -> Cop (Cload (memory_chunk, Immutable), [cmm], dbg))
