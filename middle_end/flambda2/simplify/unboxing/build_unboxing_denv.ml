...
let denv_of_number_decision naked_kind shape param_var naked_var denv : DE.t =
  (* CR tnowak: verify *)
  let naked_name =
    VB.create naked_var Shape.Uid.internal_not_actually_unique Name_mode.normal
  in
...
let rec denv_of_decision denv ~param_var (decision : U.decision) : DE.t =
    ...
    let denv =
      List.fold_left
        (fun denv ({ epa = { param = var; _ }; _ } : U.field_decision) ->
          (* CR tnowak: verify *)
          let v =
            VB.create var Shape.Uid.internal_not_actually_unique
              Name_mode.normal
          in
  ...
  | Unbox (Variant { tag; const_ctors; fields_by_tag }) ->
    ...
    (* CR tnowak: verify *)
    let tag_v =
      VB.create tag.param Shape.Uid.internal_not_actually_unique
        Name_mode.normal
    in
    ...
    let denv =
      ...
      | At_least_one { is_int; _ } ->
        (* CR tnowak: verify *)
        let is_int_v =
          VB.create is_int.param Shape.Uid.internal_not_actually_unique
            Name_mode.normal
        in
    ...
    let denv, const_ctors =
      ...
      | At_least_one { ctor = Unbox (Number (Naked_immediate, ctor_epa)); _ } ->
        (* CR tnowak: verify *)
        let v =
          VB.create ctor_epa.param Shape.Uid.internal_not_actually_unique
            Name_mode.normal
        in
