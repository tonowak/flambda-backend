...
let rec bind_rec acc exn_cont ~register_const0 (prim : expr_primitive)
  ...
  | If_then_else (cond, ifso, ifnot) ->
    ...
    let cond_result_pat =
      Bound_var.create cond_result Shape.Uid.internal_not_actually_unique
        Name_mode.normal
    in
    let ifso_cont = Continuation.create () in
    let ifso_result = Variable.create "ifso_result" in
    let ifso_result_pat =
      Bound_var.create ifso_result Shape.Uid.internal_not_actually_unique
        Name_mode.normal
    in
    let ifnot_cont = Continuation.create () in
    let ifnot_result = Variable.create "ifnot_result" in
    let ifnot_result_pat =
      Bound_var.create ifnot_result Shape.Uid.internal_not_actually_unique
        Name_mode.normal
    in
