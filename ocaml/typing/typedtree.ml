...
and 'k pattern_desc =
  ...
  | Tpat_var : Ident.t * string loc * Uid.t * value_mode -> value pattern_desc
  | Tpat_alias :
      value general_pattern * Ident.t * string loc * Uid.t * value_mode -> value pattern_desc
...
type full_bound_ident_action =
  Ident.t -> string loc -> type_expr -> Uid.t -> value_mode -> sort -> unit
